...
class _MemberFormScreenState extends ConsumerState<MemberFormScreen> {
  ...
  void initState() {
    super.initState();
    _nameController = TextEditingController(text: widget.member?['name'] as String? ?? '');
    _phoneController = TextEditingController(text: widget.member?['phone'] as String? ?? '');
    _emailController = TextEditingController(text: widget.member?['email'] as String? ?? '');
    _quickMemoController = TextEditingController(text: widget.member?['quickMemo'] as String? ?? '');
    _memoController = TextEditingController(text: widget.member?['memo'] as String? ?? '');
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      if (_quickMemoController.text.trim().isNotEmpty) 'quickMemo': _quickMemoController.text.trim(),
      if (_memoController.text.trim().isNotEmpty) 'memo': _memoController.text.trim(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(isEditing ? '회원 수정' : '회원 등록'),
      ),
